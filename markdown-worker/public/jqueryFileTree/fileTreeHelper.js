 ...
 * _ test.ps1
*/
function generateBlobsHtml(currentPath, blobs){
	
	var elements = [];
	currentPath = unescape(currentPath.replace(/\/$/, '').trim());
...
function generateFoldersHtml(currentPath, blobs){
	...
	for(var key in blobs){
		...
		var blobName = '';
		var blobPath = '';
		if (pathArray.length - 1 > level && blobs[key].indexOf(currentPath) != -1){
				blobName = pathArray[level];
				for(var i = 0; i <= level;i++) {
					blobPath += pathArray[i] + '/';
				}
	...
	for(var key in elements){
		var item = elements[key];
		var itemClass = 'directory collapsed';
		html += "<li class='" + itemClass + "'><a href='#' rel='" + item.path + "'>" + item.name + "</a></li>";
	}
	html += "</ul>";
	return html;
